<?php
// Simple first-run installer (standalone page)
$basePath = rtrim(dirname(dirname($_SERVER['SCRIPT_NAME'] ?? '/')), '/');
if ($basePath === '/' || $basePath === '\\' || $basePath === '.') { $basePath = ''; }
$hasConfig = file_exists(__DIR__ . '/../backend/config.php');
if ($hasConfig) { header('Location: ' . ($basePath ?: '/')); exit; }
...
<head>
  ...
  <title>Afterlight Installer</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
  <style> body{background:#0f1115;color:#e1e6ef} .card{background:#171a22} </style>
</head>
<body>
<div class="container py-4">
  ...
  <script>
    (function(){
      ...
      let okDb = false; let okFtp = true; let timer = null; // FTP optional
      ...
      // Prevent submit if either DB or required FTP aren't validated
      document.querySelector('form')?.addEventListener('submit', (e)=>{ if(!(okDb && okFtp)){ e.preventDefault(); test(); testFtp(); } });
      // Auto-populate Base URL (strip /install) and FTP host
